...
export default function NH() {
  ...
  const ThemSP_Onclick = (event)=>{
    ...
    axios.get(`http://localhost:8080/Data/HH/GetHH/${PNCTInfo?.mahh}`).then(res=>{
      // masp, soluong, mahoadon, dongia
      let temp2= {mahh:PNCTInfo?.mahh,mancc:PNCTInfo?.mancc,soluong:PNCTInfo?.soluong,maphieunhap:PNInfo.maphieunhap,dongia:res.data.data[0]?.gianhap,tenhh:res.data.data[0]?.tenhh}
  ...
  const ThemNHAUTO = (event)=>{
    event.preventDefault();
    
    let tempdata = JSON.parse(JSON.stringify({...PNInfo,values:Temp}));
    // console.log(SPData);
    let temp =   () => {
      axios.post(`http://localhost:8080/SYS/InsertPhieuNhapAuto`,tempdata).then((res,err)=>{
          let temp2 = async () => {
              alert("Thêm thông tin Phieu nhap thành công");
              let res = await axios.get("http://localhost:8080/SYS/GetAllPhieuNhap")
          if(res.data.data!==undefined){
          SetState({type:"AllPN",payload:res.data.data})
          SetState({type:"QLPNSTT",payload:0});
  ...
  const Handler_Onchange = (event) => {
    ...
    State.QLPNSTT !== 2 ? SetPNInfo({ ...PNInfo, [event.target.name]: event.target.value }) : SetPNInfo({ ...PNInfo, tongtien: 0, [event.target.name]: event.target.value,ngaynhap:Handler.Now_Date() })
  }
            ...
            <tbody>
              ...
              <tr>
                <th>
                  ...
                  <div className="mt-3"><select style={{ width: '50%' }} onChange={(event)=>Handler_CTOnchange(event)} name="mancc">
                  <option/>

                  <ListNCC data={NCCInfo} />
                  </select>
                  <label className=" ml-2 mr-2"> Nhà cung cấp </label>
              ...
              <tr>
                <th> Tổng tiền </th>
                <td><input className="form-control" type="text" name="tongtien" placeholder={0} id="diem" onChange={(event) => Handler_Onchange(event)} disabled /></td>
              </tr>

            </tbody></table>
          <br></br>
          <button name="them" value="Xacnhan" style={{ width: '20%' }} onClick={(event) => ThemNHAUTO(event)} > Thêm Phiếu Nhập </button>
        </form>
      </div>
    )
    //THem NV
