...
export default function HH() {
  ...
  const Handler_Onchange = (event) => {
    ...
    State.QLHHSTT !== 2 ? SetHHInfo({ ...HHInfo, [event.target.name]: event.target.value }) : SetHHInfo({ ...HHInfo, soluong: 0, [event.target.name]: event.target.value,ngaynhap:Handler.Now_Date() })
  }
